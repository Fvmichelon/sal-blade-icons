...
- Publish by tagging semver on git (README: publish + consume).
- Sync upstream families with `pnpm sync:upstream` — only `regular`, `fill`, and `duotone`.
- `thin` / `light` / `bold` are out of scope.

Based on Phosphor Icons (MIT). Keep `LICENSE` and `NOTICE`.
